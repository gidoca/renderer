...
Intersectable * getScene(void)
{
  ...
  QSharedPointer<Material> material(QSharedPointer<Material>(new PhongMaterial(kd, kd, 32)));
...
//  lights.push_back(QSharedPointer<Light>(new AreaLight(QVector3D(0.3, 0.6, 0.8), QVector3D(1, 0, 0), QVector3D(0, 0, 1), Spectrum(4, 4, 4))));
  lights.push_back(QSharedPointer<Light>(new EnvironmentMap("environmentmaps/uffizi_probe.hdr")));
  return lights;
}
